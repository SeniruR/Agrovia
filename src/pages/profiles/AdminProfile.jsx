  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        console.log('adminProfile - Backend response:', data);
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="relative">
            <img
              src={
                adminData.profileImage && adminData.profileImage.trim() !== ''
              ...
              onError={e => {
                console.error('AdminProfile - Failed to load profile image:', e.target.src);
                e.target.src = 'https://i.pinimg.com/736x/7b/ec/18/7bec181edbd32d1b9315b84260d8e2d0.jpg';
              }}
              onLoad={() => {
                console.log('AdminProfile - Profile image loaded successfully:', adminData.profileImage);
              }}
            />
            {adminData.verified && (
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
              <h1 className="text-3xl font-bold">{adminData.fullName}</h1>
              {adminData.verified && (
                <CheckCircle className="w-6 h-6 text-blue-400" />
              )}
            </div>
            <p className="text-white/90 text-lg mb-4">🛒 Trusted admin</p>
          ...
          <div className="flex flex-col gap-3">
            {!isEditing ? (
              <button
                onClick={() => navigate('/profile/admin/edit')}
          ...
          <div className="grid grid-cols-1 gap-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <InfoCard label="Full Name" value={adminData.fullName} icon={User} color="green" />
                <InfoCard label="NIC" value={adminData.nic} icon={FileText} color="blue" />
                <InfoCard label="Phone Number" value={adminData.phoneNumber} icon={Phone} color="green" />
                <InfoCard label="Email" value={adminData.email} icon={Mail} color="blue" />
                <InfoCard label="District" value={adminData.district} icon={MapPin} color="yellow" />
                <InfoCard label="Address" value={adminData.address} icon={MapPin} color="gray" />
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'contact' && (
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <InfoCard label="Phone Number" value={adminData.phoneNumber} icon={Phone} color="green" />
              <InfoCard label="Email Address" value={adminData.email} icon={Mail} color="blue" />
              <InfoCard label="Address" value={adminData.address} icon={MapPin} color="gray" />
              <InfoCard label="District" value={adminData.district} icon={MapPin} color="yellow" />
            </div>
          </div>
