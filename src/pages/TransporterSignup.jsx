...
// InputField component is now defined outside TransporterSignup to prevent re-declaration on every render.
const InputField = React.memo(({
    icon, label, name, type = 'text', required = false, options = null,
    formData, handleInputChange, errors, showPassword, showConfirmPassword,
    setShowPassword, setShowConfirmPassword, ...props
}) => {
...
const TransporterSignup = () => {
    ...
    const [formData, setFormData] = useState({
        fullName: '', email: '', district: '', nic: '', birthDate: '',
        address: '', phoneNumber: '', profileImage: null, password: '',
        confirmPassword: '', vehicleType: '', vehicleNumber: '',
        vehicleCapacity: '', capacityUnit: 'kg', maxTravelDistance: '',
        distanceUnit: 'km', licenseNumber: '', licenseExpiry: '',
        experienceYears: '', additionalInfo: '',
    });
    ...
    const vehicleTypes = ['Truck', 'Lorry', 'Van', 'Three Wheeler', 'Pickup', 'Mini Truck', 'Other'];
    const capacityUnits = ['kg', 'tons', 'liters'];
    const distanceUnits = ['km', 'miles'];
    const experienceOptions = ['Less than 1 year', '1-3 years', '3-5 years', '5-10 years', 'More than 10 years'];
    ...
    const handleInputChange = useCallback((e) => {
        ...
        // By using a functional update for setErrors, we remove the need for 'errors' in the dependency array.
    ...
    }, []); // Dependency array is now empty for better performance.
    ...
    const validateForm = () => {
        ...
        if (formData.nic && !/^([0-9]{9}[x|X|v|V]|[0-9]{12})$/.test(formData.nic)) {
    ...
    // Props that need to be passed down to the InputField component
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-8">
                        <div className="space-y-6">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={User} label="Full Name" name="fullName" required placeholder="Enter your full name" />
                                <InputField {...commonFieldProps} icon={Mail} label="Email Address" name="email" type="email" required placeholder="Enter your email address" />
                            </div>
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={MapPin} label="District" name="district" required options={districts} />
                                <InputField {...commonFieldProps} icon={FileText} label="NIC Number" name="nic" required placeholder="Enter NIC number" />
                            </div>
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={Phone} label="Phone Number" name="phoneNumber" required placeholder="Enter 10-digit phone number" />
                            </div>
                            <InputField {...commonFieldProps} icon={Home} label="Address" name="address" type="textarea" placeholder="Enter your complete address" />
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={Camera} label="Profile Image" name="profileImage" type="file" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={Truck} label="Vehicle Type" name="vehicleType" required options={vehicleTypes} />
                                <InputField {...commonFieldProps} icon={FileText} label="Vehicle Number" name="vehicleNumber" required placeholder="Enter vehicle registration number" />
                            </div>
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="flex gap-2">
                                    <div className="flex-1">
                                        <InputField {...commonFieldProps} icon={Gauge} label="Vehicle Capacity" name="vehicleCapacity" required type="number" step="0.01" placeholder="Enter capacity" />
                                    </div>
                                    <div className="w-32">
                                        <InputField {...commonFieldProps} label="Unit" name="capacityUnit" options={capacityUnits} />
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <div className="flex-1">
                                        <InputField {...commonFieldProps} icon={Ruler} label="Max Travel Distance" name="maxTravelDistance" required type="number" step="0.1" placeholder="Enter distance" />
                                    </div>
                                    <div className="w-32">
                                        <InputField {...commonFieldProps} label="Unit" name="distanceUnit" options={distanceUnits} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...commonFieldProps} icon={Award} label="License Number" name="licenseNumber" required placeholder="Enter your driving license number" />
                                <InputField {...commonFieldProps} icon={Calendar} label="License Expiry Date" name="licenseExpiry" type="date" required />
                            </div>
                            <InputField {...commonFieldProps} icon={Award} label="Years of Experience" name="experienceYears" required options={experienceOptions} />
                            <InputField {...commonFieldProps} icon={FileText} label="Additional Information" name="additionalInfo" type="textarea" placeholder="Any additional info about your logistics services" />
                        </div>

                        <div className="space-y-6">
                            <h3 className="text-xl font-semibold text-green-800 border-b border-green-200 pb-2">Security Information</h3>
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputField {...passwordFieldProps} icon={Lock} label="Password" name="password" type="password" required placeholder="Enter password (min 8 characters)" />
                                <InputField {...passwordFieldProps} icon={Lock} label="Confirm Password" name="confirmPassword" type="password" required placeholder="Confirm your password" />
                            </div>
                        </div>
